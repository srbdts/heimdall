...
class Word:
    ...
    def set_flags(self,language_models):
        ...
        foreign_languages = [lm.language for lm in language_models if not lm.language == "english"]
        for language in foreign_languages:
            if self.probs[language] > self.probs["english"]:
                ...
                if self.probs[language] - total_probs_other_languages/len(foreign_languages) >= LOGPROB_THRESHOLD:
